...
<div>
    @foreach ($question->data['options'] as $option)
        <div class="flex items-center">
            ...
            @endphp
            <div @class(['w-5 h-5 rounded mr-1', 'bg-gray-300' => !$correct, 'bg-green-500' => $correct])></div>
            @isset($option['image'])
                <x-common.image :src="App\Helpers\ImageHelper::url($option['image'])"></x-common.image>
            @endisset
            <div class='ml-1 my-2'>{!! clean($option['text']) !!}</div>
            @if ($chosen)
                <div @class(['font-semibold indent-1', 'text-red-500' => !$correct, 'text-green-500' => $correct])>- Ваша відповідь</div>
            @endif
        </div>
    @endforeach
</div>